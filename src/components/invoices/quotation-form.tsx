...
import { createQuotationSchema } from "@/application/dto/quotation.schema";
...
import { useQuotationsList } from "@/hooks/use-invoices";
...
type QuotationFormValues = z.input<typeof createQuotationSchema>;
...
export function QuotationForm({ quotation }: QuotationFormProps) {
  ...
  const form = useForm<QuotationFormValues>({
    ...
    defaultValues: {
      ...
      issueDate: quotation
        ? toDateInput(quotation.issueDate)
        : new Date().toISOString().slice(0, 10),
      validUntil: quotation
        ? toDateInput(quotation.validUntil)
  ...
  const onSubmit = form.handleSubmit(async (values) => {
    ...
    try {
      const saved =
        isEdit && quotation
          ? await patchApiData<QuotationDetail>(
              `/api/invoices/quotations/${quotation.id}`,
              values,
            )
          : await postApiData<QuotationDetail>(
              "/api/invoices/quotations",
              values,
            );
      router.push(`/invoices/quotations/${saved.id}`);
    } catch (e) {
      setError(
        e instanceof Error
          ? e.message
  ...
  return (
    <form onSubmit={onSubmit} className="mx-auto flex max-w-4xl flex-col gap-6">
      <FormSection
        ...
        description={
          isEdit
            ? `Update ${quotation?.quotationNo}`
            : "Create a new client quotation"
        }
      >
      ...
      <FormSection
        title="Line items"
        description="Add quoted services or products"
      >
        <LineItemEditor
          items={lineItems}
      ...
      <FormSection title="Notes" description="Optional terms or conditions">
        <Textarea {...form.register("notes")} />
      </FormSection>

      <div className="border-border bg-card shadow-card flex items-center justify-between rounded-2xl border p-5">
        <div>
          <p className="text-muted-foreground text-sm">Quotation total</p>
